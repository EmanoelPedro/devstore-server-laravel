<x-app-layout>
    <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
        <form action="{{route('products.create')}}" method="post" enctype='multipart/form-data'>
            ...
            <x-text-input type="text" class="block mt-1 w-full" id="name" name="name" placeholder="Product Name" value="{{$product->name}}"/>
            </div>
            <div class="mt-4 ">
                <label for="description">Product Description</label>
                <textarea class="block mt-1 w-full" name="description" id="description" cols="30" rows="10">{{$product->description}}</textarea>
            </div>
            <div class="mt-4 ">
                <label for="price">Product Price</label>
            <x-text-input type="text" class="block mt-1 w-full" id="price" name="price" placeholder="Product Price" value="{{$product->price}}"/>
            </div>
    ...
    <div class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
        <form action="{{route('products.delete',$product->id)}}" method='post'>
            ...
            <input type="hidden" id="delete" name="delete" value="{{$product->id}}">
            <div>
                <x-primary-button class="ms-4 mt-4">
                    {{ __('Delete') }}
                </x-primary-button>
    ...
    <div class="container flex flex-row flex-nowrap box-content h-50 w-screen p-4">
        @foreach ($photos as $photo)
            
                <img src="{{$photo->url}}" class="object-cover h-48 w-48 p-4" alt="{{$photo->order}}">
